...
| `deployment-artifact` | the lane's artifact check (`check-bundle`, `check-mobile-release`, `check-desktop-release`, `check-bundle-extension`) | `app.json`, `tauri.conf.json` and the manifest are not code: lint and tests pass while the artifact is unbuildable or still carries the template's identifiers. **Each of these builds or resolves something and inspects the result**: the Web and Mobile checks bundle and look at the output, the Desktop one runs `cargo metadata --locked` so a hand-written `Cargo.lock` cannot pass for a resolved tree, and `check-native` compiles the Rust side (clippy, denying warnings) because the artifact of a Tauri app IS native code. An `api/` route is both `backend` AND `deployment-artifact`. |
